...
//    vendor       array of strings containing the shader source
//    num_sources   number of string in vendor, or -1 meaning vendor is NULL-terminated
...
STB_GLPROG_DECLARE GLuint stbgl_compile_shader(GLenum type, char const **vendor, int num_sources, char *error, int error_buflen)
{
   ...
   if (num_sources < 0)
      for (num_sources = 0; vendor[num_sources] != NULL; ++num_sources)
         ;
   stbglShaderSource(shader, num_sources, vendor, NULL);
